Feature: TTP View Plan Request
  ...
  Scenario: Should return error if plan ID number is invalid
    Given a view plan request
      | customerReference | planId       |
      | customerRef1234   | planId1234yz |
    When the view plan request is sent to the ttpp service
    Then the service will return a 404

  Scenario: Should return error if both customer reference & plan ID numbers are invalid
    Given a view plan request
      | customerReference | planId       |
      | customerRef1234yx | planId1234yz |
    When the view plan request is sent to the ttpp service
    Then the service will return a 404
