...
Feature: Multiple Debt Items - Edge Cases
  ...
  Scenario: 1. 2 debts. 1 interest bearing and 1 non interest bearing
    Given a debt item
  ...
  Scenario: 2. 2 debts each with 1 payment of different amounts
    Given a debt item
      | originalAmount | interestStartDate | interestRequestedTo | mainTrans | subTrans |
      | 500000         | 2018-12-16        | 2019-04-14          | 1525      | 1000     |
    And the debt item has payment history
  ...
  Scenario: 3. 3 debts, 1 with a payment
    And a debt item
      | originalAmount | interestStartDate | interestRequestedTo | mainTrans | subTrans |
      | 500000         | 2018-12-16        | 2019-04-14          | 1525      | 1000     |
    And the debt item has no payment history
...
#    300 debts items is the max says Helen
  ...
  Scenario: 5. 2 debts, 5 payments on one of the debts
    Given a debt item
      | originalAmount | interestStartDate | interestRequestedTo | mainTrans | subTrans |
  ...
  Scenario: 6. 1 debt with payment where payment date before date created
    Given a debt item
      | originalAmount | interestStartDate | interestRequestedTo | mainTrans | subTrans |
  ...
  Scenario: 7. 1 debt with a payment amount greater than original debt amount
    Given a debt item
      | originalAmount | interestStartDate | interestRequestedTo | mainTrans | subTrans |
  ...
  Scenario: 8. 1 debt with an interest start date before the debt created
    Given a debt item
  ...
  @DTD-2216
  Scenario: 9. 2 SA debts where one has an original amount less than zero
    Given a debt item
      | originalAmount | interestStartDate | interestRequestedTo | mainTrans | subTrans |
